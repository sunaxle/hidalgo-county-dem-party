...
with open('raw_canvass.txt', 'r') as f:
    text = f.read()
...
with open('canvass_insights.md', 'w') as f:
    ...
    if total_ballots > 0:
        f.write(f"- **Absentee / Mail-in:** {total_absentee} ({total_absentee/total_ballots*100:.1f}%)\n")
        f.write(f"- **Early Voting:** {total_early} ({total_early/total_ballots*100:.1f}%)\n")
        f.write(f"- **Election Day:** {total_ed} ({total_ed/total_ballots*100:.1f}%)\n\n")
        f.write("> **Insight:** Early Voting is the dominant method. Resource allocation for GOTV efforts should heavily front-load during the Early Voting period rather than Election Day.\n\n")
    ...
    if total_votes > 0:
        f.write(f"- **Marcos Isaias Velez:** {total_velez} ({total_velez/total_votes*100:.1f}%)\n")
        f.write(f"- **Vikki Goodwin:** {total_goodwin} ({total_goodwin/total_votes*100:.1f}%)\n")
        f.write(f"- **Undervotes (Ballot Drop-off):** {total_undervotes} ({(total_undervotes/total_ballots*100):.1f}% of total ballots)\n\n")
        
        # Win margins
        df_lt_gov['Margin'] = (df_lt_gov['Velez'] - df_lt_gov['Goodwin']).abs()
        df_lt_gov['Winner'] = df_lt_gov.apply(lambda row: 'Velez' if row['Velez'] > row['Goodwin'] else ('Goodwin' if row['Goodwin'] > row['Velez'] else 'Tie'), axis=1)
        
        f.write("### Precinct Performance\n")
        f.write(f"- **Precincts won by Velez:** {len(df_lt_gov[df_lt_gov['Winner'] == 'Velez'])}\n")
        f.write(f"- **Precincts won by Goodwin:** {len(df_lt_gov[df_lt_gov['Winner'] == 'Goodwin'])}\n")
        f.write(f"- **Tied Precincts:** {len(df_lt_gov[df_lt_gov['Winner'] == 'Tie'])}\n\n")

        f.write("### Strongest Precincts for Velez (by margin)\n")
        top_velez = df_lt_gov[df_lt_gov['Winner'] == 'Velez'].sort_values('Margin', ascending=False).head(5)
        for _, row in top_velez.iterrows():
            f.write(f"- **Pct {row['Precinct']}:** Velez +{row['Margin']} ({row['Velez']} to {row['Goodwin']})\n")

        f.write("\n### Strongest Precincts for Goodwin (by margin)\n")
        top_goodwin = df_lt_gov[df_lt_gov['Winner'] == 'Goodwin'].sort_values('Margin', ascending=False).head(5)
        for _, row in top_goodwin.iterrows():
            f.write(f"- **Pct {row['Precinct']}:** Goodwin +{row['Margin']} ({row['Goodwin']} to {row['Velez']})\n")
