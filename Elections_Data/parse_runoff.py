...
with open('runoff_analysis.md', 'w') as out:
    ...
    out.write("This report analyzes the official precinct-by-precinct voting data for the most recent primary runoff.\n\n")
    
    out.write(f"**Total Registered Voters:** {df['Registered_Voters'].sum()}\n")
    out.write(f"**Total Ballots Cast:** {df['Ballots_Cast'].sum()}\n")
    overall_turnout = df['Ballots_Cast'].sum() / df['Registered_Voters'].sum() * 100 if df['Registered_Voters'].sum() > 0 else 0
    out.write(f"**Overall Turnout:** {overall_turnout:.2f}%\n\n")
    
    out.write("## Top 10 Precincts by Turnout Percentage\n")
    out.write(df_to_markdown(df.sort_values('Turnout_Pct', ascending=False).head(10)) + "\n\n")

    out.write("## Top 10 Precincts by Total Ballots Cast\n")
    out.write(df_to_markdown(df.sort_values('Ballots_Cast', ascending=False).head(10)) + "\n\n")

    out.write("## Bottom 10 Precincts by Turnout Percentage\n")
    out.write(df_to_markdown(df.sort_values('Turnout_Pct', ascending=True).head(10)) + "\n\n")
