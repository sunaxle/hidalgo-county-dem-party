...
try:
    ...
    dfs = pd.read_html('Elections_Data/VAN_Access_Exports/UserListExport-9534069555.xls')
    df = dfs[0]
...
for index, row in df.iterrows():
    ...
    if 'level 2' in row_str or 'level 3' in row_str or 'admin' in row_str or 'director' in row_str:
...
print(f"Found {len(high_level_users)} users with high-level access (Level 2, Level 3, Admin, etc):")
print("-" * 50)
