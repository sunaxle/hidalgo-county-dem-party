...
print(
    f"Running Weighted K-Means on {len(targets)} high-friction precincts to find 10 new sites..."
)
...
for iteration in range(50):
    ...
    for t in targets:
        distances = [
            haversine(t["lon"], t["lat"], c["lon"], c["lat"]) for c in centroids
        ]
    ...
    for i in range(k):
        ...
        if (
            abs(new_lon - centroids[i]["lon"]) > 0.0001
            or abs(new_lat - centroids[i]["lat"]) > 0.0001
        ):
            moved += 1
