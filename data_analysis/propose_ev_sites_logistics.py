...
for feature in geojson["features"]:
    ...
    if geom["type"] == "Polygon":
        ...
        precinct_points.append(
            {
                "lat": lat,
                "lon": lon,
                "weight": voters_weight,
                "desert_severity": props["distance_to_ev"] * voters_weight,
            }
        )
...
for _ in range(15):  # 15 iterations is usually enough
    ...
    for p in precinct_points:
        distances = [
            haversine(p["lat"], p["lon"], c["lat"], c["lon"]) for c in centroids
        ]
...
with open("data/hidalgo_grocery.geojson") as f:
...
for center in centroids:
    distances_to_hubs = [
        (haversine(center["lat"], center["lon"], g["lat"], g["lon"]), g)
        for g in grocery_nodes
    ]
...
for feature in geojson["features"]:
    ...
    if geom["type"] == "Polygon":
        ...
        sim_distances = [
            haversine(lat, lon, sc["lat"], sc["lon"]) for sc in snapped_centroids
        ]
        ...
        if current_ev is None:
            current_ev = 999
        feature["properties"]["simulated_logistics_distance"] = min(
            closest_sim_dist, current_ev
        )
...
print(
    f"Generated {k} sites. {snapped_count} sites successfully snapped to Commerce Hubs!"
)
