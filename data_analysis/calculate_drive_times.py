...
def calculate_drive_times(precinct_file, ev_locations_file, output_file):
    ...
    with open(ev_locations_file) as f:
        ev_sites = json.load(f)

    # Read our generated turnout data
    try:
        with open("data_analysis/precinct_turnout.json") as f:
    ...
    for feature in precincts["features"]:
        ...
        # Add turnout data
        raw_p_num = str(
            feature["properties"].get("PCT", feature["properties"].get("PREC", ""))
        )
        try:
        ...
        if p_num in turnout_data:
            feature["properties"]["early_voting_turnout_2026"] = turnout_data[
                p_num
            ].get("early_voting", 0)
            feature["properties"]["registered_voters"] = turnout_data[p_num].get(
                "registered_voters", 0
            )
