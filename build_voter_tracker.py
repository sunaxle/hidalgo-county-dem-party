...
if main:
    ...
    # Graph Container
    graph_container = soup.new_tag('div', style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 2rem; margin-bottom: 4rem; box-shadow: 0 10px 30px rgba(0,0,0,0.2);")
...
if body:
    ...
    document.addEventListener('DOMContentLoaded', () => {
      try {
        if (typeof voterRegistrationData !== 'undefined' && voterRegistrationData.length > 0) {
          ...
          if (chartElement) {
            ...
            const labels = voterRegistrationData.map(d => d.date);
            const data = voterRegistrationData.map(d => d.count);
          ...
          console.warn("voterRegistrationData is missing or empty.");
        }
      } catch (e) {
