...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (window.fetchVolunteerData) {
    ...
    const renderBubbles = (
      profilesData,
      titleText = "Grassroots Volunteer Directory",
    ) => {
      ...
      const nodes = profilesData.map((vol, i) => {
        ...
        let initials =
          (vol["first_name"] && vol["first_name"].charAt(0)
            ? vol["first_name"].charAt(0)
            : "") +
          (vol["last_name"] && vol["last_name"].charAt(0)
            ? vol["last_name"].charAt(0)
            : "");
      ...
      currentSimulation = d3
        ...
        .force(
          "charge",
          d3.forceManyBody().strength(profilesData.length > 500 ? -2 : 2),
        )
      ...
      // Hover tooltip
      const nodeInfo = nodeElements.append("div").attr("class", "d3-node-info");
    ...
    if (zipCodeFilterDiv) {
      const uniqueZips = [
        ...new Set(
          data
            .map((d) => (d.zipcode || "").trim())
            .filter((z) => z !== "" && z.length >= 5),
        ),
      ].sort();
    ...
    if (cdFilterDiv) {
      const createCDBtn = (label, districtStr) => {
        ...
        btn.onclick = () => {
          ...
          document
            .querySelectorAll(".cd-btn")
            .forEach((b) => b.classList.remove("active"));
          btn.classList.add("active");

          // Clear other global filters visually (A-Z and Zip)
          document
            .querySelectorAll(".zip-btn, [class*='alphabet-btn']:not(.cd-btn)")
            .forEach((b) => b.classList.remove("active"));
