document.addEventListener("DOMContentLoaded", async () => {
  ...
  const stylePrecinct = (feature) => {
    let distance = isSimulated
      ? feature.properties.simulated_distance || 0
      : feature.properties.distance_to_ev || 0;
  ...
  try {
    const [geojsonRes, evLocRes, evTotal2024, evTotal2026, simLocRes] =
      await Promise.all([
    ...
    const renderPrecincts = () => {
      ...
      geojsonLayer = L.geoJson(precinctsGeojson, {
        ...
        onEachFeature: (feature, layer) => {
          const dist =
            (isSimulated
              ? feature.properties.simulated_distance
              : feature.properties.distance_to_ev) || 0;
          const pct =
            feature.properties.PCT || feature.properties.PREC || "Unknown";
          const site = feature.properties.nearest_ev_site || "None";
          const rv = feature.properties.registered_voters || 0;
          const penalty = isSimulated
            ? feature.properties.simulated_penalty
            : feature.properties.collective_mileage_penalty;
    ...
    simLocations.features.forEach((site) => {
      ...
      L.circleMarker(latlng, {
        ...
        .bindTooltip("<strong>" + site.properties.name + "</strong>")
    ...
    const renderCircles = (turnoutDataset, electionLabel) => {
      ...
      document.getElementById("turnout-label").innerText =
        `${electionLabel} Turnout`;
      ...
      evLocations.features.forEach((site) => {
        ...
        for (let [key, val] of Object.entries(turnoutDataset)) {
          if (
            locName.toLowerCase().includes(key.toLowerCase().substring(0, 8))
          ) {
            matchedTurnout = val;
            totalVals += val;
            countVals++;
      ...
      document.getElementById("stat-new-votes").innerText =
        `+${(avgTurnout * 10).toLocaleString()}`;
    };
    ...
    const buildTable = () => {
      ...
      precinctsGeojson.features.forEach((f) => {
        ...
        const dist = isSimulated
          ? f.properties.simulated_distance
          : f.properties.distance_to_ev;
        const penalty = isSimulated
          ? f.properties.simulated_penalty
          : f.properties.collective_mileage_penalty;
      ...
      if (deserts.length > 0) {
        ...
        document.getElementById("stat-avg-dist").innerText = (
          totalDist / deserts.length
        ).toFixed(1);
      }
    };
    buildTable();
