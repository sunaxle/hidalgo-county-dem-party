document.addEventListener("DOMContentLoaded", () => {
  ...
  const stylePrecinct = (feature) => {
    ...
    if (feature.properties.CD === "28") {
      cdColor = "#a855f7";
      cdWeight = 2;
    }

    let dist = isSimulated
      ? feature.properties.simulated_logistics_distance ||
        feature.properties.distance_to_ev
      : feature.properties.distance_to_ev;
    return {
  ...
  // Load Data
  Promise.all([
    fetch("data_analysis/hidalgo_logistics_precincts.geojson").then((r) =>
      r.json(),
    ),
    fetch("data/hidalgo_grocery.geojson").then((r) => r.json()),
    fetch("data_analysis/logistics_sites.json").then((r) => r.json()),
    fetch("hidalgo-election-map/polling_locations.geojson").then((r) =>
      r.json(),
    ),
    ...
    // Draw Precincts
    precinctsLayer = L.geoJson(geojsonData, { style: stylePrecinct }).addTo(
      map,
    );
    ...
    document.getElementById("toggle-sim").addEventListener("click", (e) => {
      ...
      if (isSimulated) {
        ...
        if (!newSitesLayer) {
          ...
          snappedSites.forEach((s) => {
            ...
            let note = s.is_snapped
              ? `Snapped to: ${s.snapped_to}`
              : `Strict K-Means Centroid (No Groceries near)`;
      ...
      precinctsLayer.eachLayer((layer) =>
        layer.setStyle(stylePrecinct(layer.feature)),
      );
    });

    document
      .getElementById("toggle-commerce")
