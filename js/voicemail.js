      ...
      try {
        ...
        mediaRecorder.onstop = async () => {
          ...
          recordStatus.innerHTML =
            '<span style="color: #fcd34d;">Uploading and transcribing... ⏳</span>';
          ...
          try {
            ...
            const storageRef = ref(
              storage,
              `voicemails/voicemail_${timestamp}.webm`,
            );

            const metadata = {
              customMetadata: {
                firstName: document.getElementById("vm-first-name")
                  ? document.getElementById("vm-first-name").value
                  : "",
                lastName: document.getElementById("vm-last-name")
                  ? document.getElementById("vm-last-name").value
                  : "",
                phone: document.getElementById("vm-phone")
                  ? document.getElementById("vm-phone").value
                  : "",
                email: document.getElementById("vm-email")
            ...
            recordStatus.innerHTML =
              '<span style="color: #4ade80;">Success! Your voice message has been sent to the Chairman. ✅</span>';
          } catch (error) {
            console.error("Upload failed", error);
            recordStatus.innerHTML =
              '<span style="color: #ef4444;">Upload failed. Please check your connection.</span>';
          }
        ...
        recordStatus.innerHTML =
          '<span style="color: #ef4444; font-weight: bold;">Recording... Tap to stop. 🔴</span>';
      } catch (err) {
        console.error("Microphone access denied or error:", err);
        recordStatus.innerHTML =
          '<span style="color: #ef4444;">Microphone access denied. Please allow permissions.</span>';
      }
    } else {
      // Stop Recording
      mediaRecorder.stop();
