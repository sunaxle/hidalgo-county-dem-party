  ...
  fetch("data/precincts.json")
    ...
    .then(async (data) => {
      ...
      if (typeof chairDataList2026 !== "undefined") {
        const liveProfile = chairDataList2026.find(
          (d) =>
            String(d.precinct) === String(precinctId) &&
            (d.role === "Precinct Chair" || d.role === "Neighborhood Captain"),
        );
        if (liveProfile) {
          if (!precinctData) {
            precinctData = {
              precinct: precinctId,
              announcement:
                "Welcome to our new precinct page! Check back soon for localized updates.",
      ...
      const GOOGLE_SHEET_CSV =
        "https://docs.google.com/spreadsheets/d/e/2PACX-1vQl7PmDwc7YoEi0u_lZL1ErXCP03mZhPFhk6--nBynTSn6hahLGaL4nZC0FsfpvAoAPVHU91tp3RVZJ/pub?output=csv";
      ...
      try {
        if (typeof d3 !== "undefined") {
          ...
          for (let i = 0; i < liveData.length; i++) {
            ...
            const pctKey =
              "Precinct Number (Critically important, this is how the database knows who is who)";
            ...
            if (String(row[pctKey]).trim() === String(precinctId)) {
              ...
              const photoKey =
                "Profile Picture Link (Have them paste an image URL, or use the Form's File Upload and we'll parse the Google Drive link)";
              ...
              if (
                row["Latest Announcement"] &&
                row["Latest Announcement"].trim() !== ""
              ) {
              ...
              if (
                row["Next Meeting Time"] &&
                row["Next Meeting Time"].trim() !== ""
              ) {
                precinctData.meeting_time = row["Next Meeting Time"];
              }
              if (
                row["Next Meeting Location"] &&
                row["Next Meeting Location"].trim() !== ""
              ) {
          ...
          if (custom_announcements.length > 0) {
            precinctData.announcement_html = custom_announcements
              .reverse()
              .join(
                '<br><hr style="border:0; border-top:1px dashed #cbd5e1; margin: 12px 0;"><br>',
              );
          }
        }
      } catch (e) {
        console.warn(
          "Could not load or parse Google Sheet CSV. Reverting to static arrays safely.",
          e,
        );
      }
