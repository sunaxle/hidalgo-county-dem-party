...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    typeof chairDataList === "undefined" ||
    typeof precinctDistricts === "undefined"
  ) {
    console.error(
      "Required data arrays (chairDataList or precinctDistricts) are not loaded.",
    );
    return;
  }
  ...
  chairDataList.forEach((person) => {
    ...
    if (window.fetchVolunteerData) {
      window.fetchVolunteerData().then((volunteers) => {
        const generalVols = volunteers.filter(
          (v) => v["Role"].trim() === "General Volunteer",
        );
        ...
        if (matches.length > 0) {
          ...
          alertBox.style =
            "background: rgba(251, 191, 36, 0.15); border: 2px solid #fbbf24; color: #fcd34d; padding: 1.5rem; border-radius: 8px; margin-bottom: 2.5rem; text-align: center; font-size: 1.1rem;";
  ...
  vacantPrecincts.forEach((gap) => {
    ...
    if (commissionerBuckets[cc] !== undefined)
      commissionerBuckets[cc].push(gap.precinct);
    ...
    if (cityName === "mcallen") {
      ...
      ["weslaco", "mercedes", "donna", "edcouch", "elsa", "la villa"].includes(
        cityName,
      )
    ) {
  ...
  renderBucket(
    "chips-city-midvalley",
    cityBuckets["midvalley"],
    "badge-city-midvalley",
  );
  ...
  if (window.fetchVolunteerData) {
    window.fetchVolunteerData().then((volunteers) => {
      ...
      if (matches.length > 0) {
        ...
        alertBox.style =
          "background: rgba(251, 191, 36, 0.15); border: 2px solid #fbbf24; color: #fcd34d; padding: 1.5rem; border-radius: 8px; margin-bottom: 2.5rem; text-align: center; font-size: 1.1rem;";
        alertBox.innerHTML = `<strong>🚨 URGENT:</strong> You have ${matches.length} General Volunteer(s) residing in completely vacant precincts! <br><a href="volunteer_roster.html" style="color: #fbbf24; text-decoration: underline;">Open the Volunteer Roster to identify and promote them.</a>`;
