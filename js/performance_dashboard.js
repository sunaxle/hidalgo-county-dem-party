document.addEventListener("DOMContentLoaded", () => {
  ...
  function initCharts() {
        ...
        datasets: [
          {
            data: [
              stats.totalTarget,
              Math.max(0, COUNTY_GOAL - stats.totalTarget),
            ],
            backgroundColor: [
    ...
    // Update Text Display
    document.getElementById("goalTargetText").innerText =
  ...
  function updateCharts(filter) {
    ...
    // Update Bar Chart
    voterChart.data.datasets[0].data = [
      stats.totalRegistered,
      stats.totalPastPrimary,
    ];
    voterChart.update();
    ...
    animateValue(
      targetEl,
      parseInt(targetEl.innerText.replace(/,/g, "")),
      stats.totalTarget,
      500,
    );
  ...
  function updateVacantTable() {
    ...
    const sortedData = [...perfData].sort(
      (a, b) => parseInt(a.precinct) - parseInt(b.precinct),
    );
