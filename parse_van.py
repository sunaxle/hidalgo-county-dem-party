...
VAN_EXPORT_PATH = os.path.join(BASE_DIR, 'Elections_Data', 'VAN_Access_Exports', 'UserListExport-9534069555.xls')
REPORT_PATH = os.path.join(BASE_DIR, 'Elections_Data', 'VAN_Access_Exports', 'van_cross_reference_report.md')
...
try:
    with open(CHAIR_DATA_PATH, 'r', encoding='utf-8') as f:
        js_content = f.read()
    ...
    for line in lines:
        ...
        name_match = re.search(r'(?:name|Name|chair|Chair)\s*:\s*[\'"]([^\'"]+)[\'"]', line)
        if name_match:
...
try:
    ...
    with open(REPORT_PATH, 'w', encoding='utf-8') as out:
        ...
        out.write(f"**Non-Chairs/Staff/Volunteers with VAN Access:** {len(non_chairs_with_access)}\n\n")
