...
def detect_google_drive_path():
    if settings.google_drive_path:
        return settings.google_drive_path
...
def transcribe_audio(audio_path):
    try:
        ...
        print(
            "   To enable voice-note transcription, run this in your Antigravity terminal:"
        )
...
def monitor_dropzone():
    ...
    if not drive_path:
        ...
        print(
            "   Please manually set the GOOGLE_DRIVE_PATH variable at the top of this script."
        )
        return

    dropzone_path = os.path.join(drive_path, settings.dropzone_relative_path)
    ...
    while True:
        try:
            ...
            for file in os.listdir(dropzone_path):
                ...
                if ext.lower() in settings.audio_extensions and "_processed" not in name:
                    ...
                    if transcript_text:
                        ...
                        os.rename(
                            audio_file_path, os.path.join(dropzone_path, archive_name)
                        )
            ...
            for file in os.listdir(dropzone_path):
                if (
                    file.endswith(".json")
                    and not file.startswith("instructions_processed")
                    and not file.startswith("instructions_failed")
                    and not file.startswith("instructions_template")
                    and not file.startswith("instructions_old")
                    and file != "test_template.txt"
                ):
                    ...
                    try:
                        ...
                        archive_name = (
                            f"instructions_processed_{int(time.time())}_{file}"
                        )
