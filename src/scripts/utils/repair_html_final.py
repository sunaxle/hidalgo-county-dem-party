...
for i, line in enumerate(lines):
    ...
    if (
        start_idx != -1
        and i > start_idx
        and '<p class="official-title">206th State District Court</p>' in line
    ):
        end_idx = i - 2
        break
...
# 2. Get HEAD file
head_file = subprocess.check_output(
    ["git", "show", "HEAD:elected_officials.html"]
).decode("utf-8")
...
restored_lines = (
    head_lines[:section_end_idx]
    + [line.rstrip() for line in local_tab_lines]
    + head_lines[section_end_idx:]
)
