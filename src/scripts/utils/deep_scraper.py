...
# Read URLs
with open(
    "/Users/dr3/.gemini/antigravity/brain/5b53f512-c9a5-4fdd-b1e0-acdcbb2d9a4b/active_county_websites.md",
) as f:
    text = f.read()
...
with open(
    "/Users/dr3/.gemini/antigravity/brain/5b53f512-c9a5-4fdd-b1e0-acdcbb2d9a4b/deep_scan_results.md",
    "w",
) as out:
    out.write("# Deep AI Web Crawl: All 54 Texas County Sites\n\n")
    out.write(
        f"**Total Successfully Scraped:** {len(results)} out of {len(urls)} live websites.\n\n"
    )

    out.write("## 🏆 Rare & Unique Pages (The Insights!)\n")
    out.write(
        "Here are highly specific pages/features that only 2 to 6 of the 54 county parties possess. These are unique strategies we could steal:\n\n"
    )
