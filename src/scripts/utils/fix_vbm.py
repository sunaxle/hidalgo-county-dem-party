...
nav_match = re.search(
    r'(<nav class="tx-clone-nav">.*?<div class="tx-clone-nav-accent-bar"></div>)',
    html,
    re.DOTALL,
)
footer_match = re.search(
    r'(<footer class="tx-clone-footer">.*?</html>)', html, re.DOTALL
)
...
with open("vbm.html", encoding="utf-8") as f:
    vbm_full = f.read()

# Extract just the main content
main_match = re.search(
    r'(<header class="page-header container fade-in" style="text-align: center; padding: 4rem 2rem;">.*?</main>)',
    vbm_full,
    re.DOTALL,
)
