...
try:
    with open("data/mock_volunteers.csv") as f:
        content = f.read()
    ...
    chairs = [
        d for d in data if d.get("Role", "") in ["Precinct Chair", "Block Captain"]
    ]
    print(f"Parsed {len(data)} rows successfully!")
    print(f"Found {len(chairs)} chairs/captains.")
    if chairs:
        print(
            f"Example: {chairs[0]['First Name']} {chairs[0]['Last Name']} ({chairs[0]['Role']} Pct {chairs[0]['Precinct Number']})"
        )
