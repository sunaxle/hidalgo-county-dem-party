...
def main():
    ...
    safe_picks = [
        f
        for f in files
        if "for Volunteers" in f
        or "Purple Geometric" in f
        or "Insta" in f
        or "Story" in f
    ]
    ...
    # Add the 3 core ones
    image_data_strs.append(
        "      { url: 'images/blockwalk.jpg', title: 'Blockwalking', desc: 'Connecting with voters' }"
    )
    image_data_strs.append(
        "      { url: 'images/community.jpg', title: 'Community Outreach', desc: 'Building relationships' }"
    )
    image_data_strs.append(
        "      { url: 'images/meeting.jpg', title: 'Strategy Meeting', desc: 'Planning the road ahead' }"
    )
    ...
    for f in selected:
        ...
        image_data_strs.append(
            f"      {{ url: '{url_path}', title: '{title}', desc: 'Action Album' }}"
        )
