...
def process_files():
    with open("state_party_clone.html", encoding="utf-8") as f:
        master_html = f.read()

    nav_match = re.search(
        r'<nav class="tx-clone-nav">.*?</nav>\s*<div class="tx-clone-nav-accent-bar"></div>',
        master_html,
        re.DOTALL,
    )
    footer_match = re.search(
        r'<footer class="tx-clone-footer">.*?</footer>', master_html, re.DOTALL
    )
...
""".replace("\\", "\\\\")
    ...
    for root, dirs, files in os.walk("."):
        ...
        if "Demographic data requests" in dirs:
            dirs.remove("Demographic data requests")

        for file in files:
            if file.endswith(".html") and file not in [
                "state_party_clone.html",
                "precinct.html",
            ]:
