...
def inject_profiles_nav():
    ...
    for file_path in html_files:
        ...
        if (
            target_str in content
            and "precinct_profiles.html"
            not in content[content.find(target_str) : content.find(target_str) + 200]
        ):
    ...
    print(
        f"Successfully injected the Precinct Profiles link into {updated_count} HTML files!"
    )
