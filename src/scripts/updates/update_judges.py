...
for line in lines[1:]:
    ...
    if "SDC" in office:
        ...
        full_office = office.replace("CCL", "County Court at Law No.").replace(
            "No. ", "No. "
        )
        if office == "CCL6":
...
losers = [
    ("Esparza", "Lawrence"),
    ("Zambrano", "Jorge Luis"),
    ("Treviño", "Sonia Melissa"),
    ("Hernández", "Victor Luis"),
]

filtered = []
for off in officials:
    should_remove = False
    for lname, fname in losers:
        if off["last_name"] == lname and fname in off["first_name"]:
            should_remove = True
            break
...
for off in filtered:
    ...
    # Add notes
    note = ""
    # Katherine Garcia Perez
    if off["last_name"] == "García Pérez" and "Katherine" in off["first_name"]:
        note = ' <span style="font-size:0.75rem; font-weight: 600; background:rgba(251, 191, 36, 0.2); color:#fbbf24; padding:2px 8px; border-radius:12px; margin-left:8px; vertical-align:middle; border:1px solid #fbbf24;">Candidate-Elect</span>'
    # Aissa Ilianna Garza
    elif off["last_name"] == "Garza" and "Aissa" in off["first_name"]:
        note = ' <span style="font-size:0.75rem; font-weight: 600; background:rgba(251, 191, 36, 0.2); color:#fbbf24; padding:2px 8px; border-radius:12px; margin-left:8px; vertical-align:middle; border:1px solid #fbbf24;">Candidate-Elect</span>'
    # Fred Garza Jr
    elif off["last_name"] == "Garza, Jr." and "Federico" in off["first_name"]:
        note = ' <span style="font-size:0.75rem; font-weight: 600; background:rgba(248, 113, 113, 0.2); color:#f87171; padding:2px 8px; border-radius:12px; margin-left:8px; vertical-align:middle; border:1px solid #f87171;">Outgoing (Term ends Jan)</span>'
    ...
    card = f"""
        <div class="official-card">
          ...
          <p class="official-title">{title}{note}</p>
        </div>"""
...
# Read elected_officials.html
with open("elected_officials.html", encoding="utf-8") as f:
    html = f.read()

if 'id="tab-local"' in html:
    html = re.sub(
        r'<!-- Local & Judicial Tab -->\s*<div class="tab-content" id="tab-local">.*?</div>\s*</div>',
        new_tab_content.strip(),
        html,
        flags=re.DOTALL,
    )
else:
    # Append if not there (which should not be the case because we injected it previously)
    pass

with open("elected_officials.html", "w", encoding="utf-8") as f:
    f.write(html)

print("Re-Updated judges in elected_officials.html")
