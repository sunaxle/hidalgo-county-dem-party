...
new_html = (
    html[: precinct_match.start(1)]
    + new_precinct_json
    + html[precinct_match.end(1) : uncontested_match.start(1)]
    + new_uncontested_json
    + html[uncontested_match.end(1) :]
)

# Now string replacements for the logic and the text
new_html = new_html.replace(
    "const cdContested = precinctData.filter(d => d.CD === cdVal);",
    "const cdContested = precinctData.filter(d => String(d.CD).includes(cdVal));",
)
new_html = new_html.replace(
    "const cdUncontested = uncontestedData.filter(d => d.CD === cdVal);",
    "const cdUncontested = uncontestedData.filter(d => String(d.CD).includes(cdVal));",
)

# We need to replace the old hardcoded text "19.02%" and "19.04%" with the new means.
new_html = re.sub(
    r'<div class="metric-value"[^>]*>19\.02%</div>',
    f'<div class="metric-value" style="color: #eab308;">{cd15_mean:.2f}%</div>',
    new_html,
)
new_html = re.sub(
    r'<div class="metric-value"[^>]*>19\.04%</div>',
    f'<div class="metric-value" style="color: #eab308;">{cd28_mean:.2f}%</div>',
    new_html,
)

new_html = new_html.replace(
    "CD15: 19.02% | CD28: 19.04%", f"CD15: {cd15_mean:.2f}% | CD28: {cd28_mean:.2f}%"
)
