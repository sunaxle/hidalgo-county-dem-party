...
try:
    with open("dev-tools/van_turfs.json") as f:
...
def send_email(
    ...
    if precinct_num == "Unknown":
        ...
        html_content = template_html.replace(
            "Precinct {{PRECINCT_NUMBER}}", "Your Area"
        )
        html_content = html_content.replace(
            "the designated Precinct Chair for Precinct {{PRECINCT_NUMBER}}",
            "a Democratic Leader in our community",
        )
        # Side-step the specific voter count logic
        html_content = html_content.replace(
            "<p>Your precinct currently has</p>",
            "<p>Our powerful community is filled with</p>",
        )
        html_content = html_content.replace(
            "{{VAN_VOTER_COUNT}} Registered Democrats", "Thousands of Active Democrats"
        )
        html_content = html_content.replace(
            "If you bring just 1% of them to the marcha, we will overwhelmingly exceed our goals.",
            "If you activate just a few people in your network for the marcha, we will overwhelmingly exceed our goals.",
        )
    else:
        subject = (
            f"Action Required: Join the LUPE May 2nd Marcha (Precinct {precinct_num})"
        )
    ...
    if is_dry_run:
        print(
            f"[DRY RUN] Would send to: {recipient_name} ({recipient_email}) for Precinct {precinct_num}"
        )
...
if __name__ == "__main__":
    ...
    sent_emails = set()
    try:
        with open("dev-tools/log_success.txt") as f:
            for line in f:
    ...
    try:
        with (
            open(CHAIRS_CSV, encoding="utf-8") as f,
            open("dev-tools/log_success.txt", "a") as log_f,
        ):
