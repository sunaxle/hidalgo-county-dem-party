...
with open("new_chairs.tsv") as f:
    for line in f:
        ...
        address = cols[6].strip() if len(cols) > 6 else ""
        city = cols[7].strip() if len(cols) > 7 else ""
...
# Wait, currently chair_data.js has the PREVIOUS run. We can still extract Block Captains!
...
block_captains = [c for c in old_data if "Block Captain" in c.get("role", "")]
...
new_js_content = (
    js_content[: match.start(1)] + new_json_str + js_content[match.end(1) :]
)
...
print(
    f"Successfully processed {len(new_chairs)} chairs with address data and kept {len(block_captains)} block captains."
)
