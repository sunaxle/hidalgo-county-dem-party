...
def get_icon(office_full):
    if "Senator" in office_full or "CD " in office_full:
        ...
        "Governor" in office_full
        or "General" in office_full
        or "Commissioner" in office_full
        or "SBOE" in office_full
    ):
        return "⭐"
        ...
        "Court" in office_full
        or "CCL" in office_full
        or "JD" in office_full
        or "Attorney" in office_full
        or "Judge" in office_full
        or "Clerk" in office_full
    ):
        return "⚖️"
...
html = re.sub(
    r'<!-- U\.S\. Congress Tab -->\s*<div class="tab-content(?:\s+active)?" id="tab-federal">.*?</div>\s*</div>',
    new_fed,
    html,
    flags=re.DOTALL,
)
html = re.sub(
    r'<!-- Texas Legislature Tab -->\s*<div class="tab-content" id="tab-state">.*?</div>\s*</div>',
    new_state,
    html,
    flags=re.DOTALL,
)
