...
try:
    with open("precinct_profiles.html", encoding="utf-8") as f:
        text = f.read()

    text = text.replace(
        "Precinct Profiles | Hidalgo Dems", "Volunteer Roster | Hidalgo Dems"
    )
    text = text.replace(
        '<h1 style="color: var(--accent); font-size: 3rem; margin-bottom: 1rem;">Precinct Profiles</h1>',
        '<h1 style="color: var(--accent); font-size: 3rem; margin-bottom: 1rem;">Volunteer Roster</h1>',
    )
    text = text.replace(
        ...
        "Access our live volunteer database. Search for your own generated profile by typing your first and last name below to connect with your designated Precinct Chair.",
    )
