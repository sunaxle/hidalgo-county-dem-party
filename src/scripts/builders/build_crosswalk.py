...
if df_hd is not None:
    df_hd = df_hd.rename(
        columns=lambda x: "HD" if "STATE HOUSE" in x or "HOUSE" in x else x
    )
...
if df_sd is not None:
    df_sd = df_sd.rename(
        columns=lambda x: "SD" if "STATE SENATE" in x or "SENATE" in x else x
    )
...
if df_cc is not None and df_hd is not None and df_sd is not None:
    ...
    merged_df = pd.merge(
        df_cc[["PRECINCT", "CC"]], df_hd[["PRECINCT", "HD"]], on="PRECINCT", how="outer"
    )
    master_df = pd.merge(
        merged_df, df_sd[["PRECINCT", "SD"]], on="PRECINCT", how="outer"
    )
