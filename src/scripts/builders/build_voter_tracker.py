...
with open("vdr_portal.html", encoding="utf-8") as f:
...
if main:
    ...
    section_tracker = soup.new_tag(
        "div", attrs={"class": "party-section", "style": "margin-top: 2rem;"}
    )
    ...
    card1 = soup.new_tag(
        "div",
        attrs={"class": "civics-card", "style": "text-align: center; cursor: default;"},
    )
    ...
    c1_val = soup.new_tag(
        "div",
        id="current-count",
        style="font-size: 3rem; font-weight: 800; color: #38bdf8; font-family: 'Alfa Slab One', cursive;",
    )
    c1_val.string = "---,---"
    ...
    c2_val = soup.new_tag(
        "div",
        id="low-goal",
        style="font-size: 3rem; font-weight: 800; color: #f472b6; font-family: 'Alfa Slab One', cursive;",
    )
    ...
    c3_val = soup.new_tag(
        "div",
        id="high-goal",
        style="font-size: 3rem; font-weight: 800; color: #34d399; font-family: 'Alfa Slab One', cursive;",
    )
    ...
    # Graph Container
    graph_container = soup.new_tag(
        "div",
        style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 2rem; margin-bottom: 2rem; box-shadow: 0 10px 30px rgba(0,0,0,0.2);",
    )
    ...
    table_container = soup.new_tag(
        "div",
        style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: 2rem; margin-bottom: 4rem; box-shadow: 0 10px 30px rgba(0,0,0,0.2); overflow-x: auto;",
    )
    table_title = soup.new_tag(
        "h3",
        style="color: white; font-family: 'Montserrat', sans-serif; font-size: 1.5rem; margin-bottom: 1.5rem; text-align: center;",
    )
    ...
    vdr_callout = soup.new_tag(
        "div",
        style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 24px; padding: 3rem 2rem; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.2);",
    )
    v_h3 = soup.new_tag(
        "h3",
        style="color: #34d399; font-size: 1.75rem; margin-bottom: 1rem; font-family: 'Montserrat', sans-serif; font-weight: 800;",
    )
    v_h3.string = "Become a Volunteer Deputy Registrar"
    v_p = soup.new_tag(
        "p",
        style="font-size: 1.15rem; line-height: 1.6; color: #cbd5e1; margin-bottom: 2rem; max-width: 800px; margin-left: auto; margin-right: auto;",
    )
    ...
    v_a = soup.new_tag(
        "a",
        href="vdr_portal.html",
        style="display: inline-block; padding: 1rem 3rem; background: linear-gradient(45deg, #34d399, #10b981); color: white; border-radius: 30px; font-family: 'Montserrat', sans-serif; font-weight: 800; font-size: 1.1rem; text-decoration: none; box-shadow: 0 10px 20px rgba(0,0,0,0.3); transition: transform 0.3s ease;",
    )
