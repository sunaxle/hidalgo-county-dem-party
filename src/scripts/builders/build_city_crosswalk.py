...
def main():
    ...
    with open(city_file, encoding="utf-16") as f:
        html = f.read()
    ...
    json_str = (
        js_content.replace("const precinctDistricts = ", "").replace(";", "").strip()
    )
    ...
    print(
        f"Successfully updated {updated_count} out of {len(data)} precincts in precinct_mapping_data.js"
    )
