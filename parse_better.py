...
def parse_xml(file_path, year):
    ...
    for event, elem in context:
        ...
        if event == 'start' and tag == 'Worksheet':
            current_sheet = elem.attrib.get('{urn:schemas-microsoft-com:office:spreadsheet}Name', 'Unknown')
            row_count = 0
            header = []
            ...
            if current_row and (current_row[0] == "107" or current_row[0] == "0107"):
                ...
                elif "President" in current_sheet or "Governor" in current_sheet or "Senator" in current_sheet or "US Rep" in current_sheet or "U.S. Rep" in current_sheet:
                    print(f"Sheet: {current_sheet}")
                    print(f"  Header: {header}")
                    print(f"  107   : {current_row}")
            elem.clear()
