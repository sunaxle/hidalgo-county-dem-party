...
def dig_into_category(category_name, category_url):
    ...
    for link in soup.find_all("a"):
        ...
        if (
            href
            and len(text) > 4
            and "/ArchiveCenter/ViewFile" not in href
            and "/DocumentCenter/" not in href
        ):
            ...
            elif "/1" in href and not href.startswith(
                "http"
            ):  # Internal routing link e.g. /1725/John-Doe
                candidate_links[text] = urljoin(BASE_URL, href)
...
def download_candidate_reports(candidate_name, candidate_url):
    ...
    candidate_folder = os.path.join(
        "downloads", candidate_name.replace(" ", "_").replace(",", "")
    )
    ...
    if pdf_count > 0:
        print(
            f"   ✅ Successfully extracted {pdf_count} financial reports for {candidate_name}"
        )
    else:
