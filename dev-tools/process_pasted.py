...
def parse_pasted_contacts(input_txt, existing_csv, output_csv):
    ...
    try:
        ...
        print(
            f"Warning: {existing_csv} not found. Proceeding without deduplicating against it."
        )
    ...
    with open(output_csv, mode="w", encoding="utf-8", newline="") as outfile:
        writer = csv.DictWriter(
            outfile, fieldnames=["FirstName", "LastName", "Email", "Tags"]
        )
        writer.writeheader()
    ...
    print(
        f"🔥 These {len(unique_new_contacts)} unique contacts have been saved to {output_csv}"
    )
