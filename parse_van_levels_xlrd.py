...
try:
    ...
    df = pd.read_excel('Elections_Data/VAN_Access_Exports/UserListExport-9534069555.xls')
except Exception as e:
    print(f"Failed to read as Excel: {e}")
    try:
        # Fallback to reading it with a different engine or trying to decode it
        import chardet
        with open('Elections_Data/VAN_Access_Exports/UserListExport-9534069555.xls', 'rb') as f:
            raw_data = f.read(1000)
            result = chardet.detect(raw_data)
            encoding = result['encoding']
        
        # If it's a tab-separated fake XLS
        df = pd.read_csv('Elections_Data/VAN_Access_Exports/UserListExport-9534069555.xls', sep='\t', encoding=encoding, on_bad_lines='skip')
...
for index, row in df.iterrows():
    ...
    if 'level 2' in row_str or 'level 3' in row_str or 'level-2' in row_str or 'level-3' in row_str or 'admin' in row_str:
